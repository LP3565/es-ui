<template>
  <Transition name="alert">
    <div :class="[b(), m(mold), m(variant)]" v-if="isShow" :style="style">
      <div :class="e('icon')">
        <svg
      ...
      <div :class="e('describe')">
        ...
        <span :class="e('description')" v-if="$slots.default">
          <slot />
        </span>
      </div>

      <div :class="e('clear')" v-if="clear" @click="clearHandle">
...
const { b, e, m } = useNamespace('alert')

const clearHandle = (event: MouseEvent) => {
  emit('clear', event)
  isShow.value = false
}
...
const style = computed<StyleValue>(() => {
  if (props.variant === 'filled') {
    return { backgroundColor: variantStyle.value, color: '#fff' }
  } else if (props.variant === 'outlined') {
    return {
      backgroundColor: 'transparent',
      color: variantStyle.value,
      border: `1px solid ${variantStyle.value}`,
    }
  } else {
    return {}
  }
