...
export const useNamespace = (block: string) => {
  ...
  const be = (blockSuffix?: string, element?: string) => (blockSuffix && element ? _bem(block, blockSuffix, element, '') : '')
  const bm = (blockSuffix?: string, modifier?: string) => (blockSuffix && modifier ? _bem(block, blockSuffix, '', modifier) : '')
